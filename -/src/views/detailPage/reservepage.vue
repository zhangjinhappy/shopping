<template>
    <div class="pic">
        ...
        <div class="shop-body">
            <div class="body-r">
                <h1 :style="{'font-size':'15px','font-weight':'600','color':'#FD5C63','margin':'5px'}">
                    ￥{{goodsDetail.showprice}} <span class="xbq">火爆促销</span>
        ...
        <div class="shop-body">
            <span style="color:#333333;margin-left: 6px;padding: 0px 15px 6px 0;">邮费：<i style="color:#F5A623">{{goodsDetail.freight===0?"免邮费":"￥"+goodsDetail.freight}}</i></span>
            <div class="line-r">
                月销量{{goodsDetail.ordercount}}
            </div>
            <span style="color:#333333">{{goodsDetail.provincecity}}</span>
        </div>
        <div class="shop-body" style="margin: 5px 0;display: block;">
            <div style="margin: 1px 0 10px 0" v-if="goodsDetail.shareprofit!=0">推广 <span class="fw xbq">分享可赚取{{goodsDetail.shareprofit}}</span></div>
            <div style="margin: 10px 0 5px 0">服务 <span class="fw">无忧退货 7天无理由 公益宝贝</span></div>
        </div>

        <div class="shop-body" style="height:48px;" v-if="goodsDetail.senddate!=''">
            <h2 class="person">发货时间</h2>
            <h1 class="number">{{goodsDetail.senddate}}</h1>
        </div>

        <div class="shop-body" style="height:48px;margin: 5px 0;" @click="shopcomment(id)">
        ...
        <!--商品品 图片详情 -->
        <div>
            <img class="page" v-for="img in goodsImage" v-lazy="img.imageurl" alt="商品详情图">
        </div>
        ...
        <div class="words_explain">
            ...
            <div>
                ...
                <div class="words_explain_second"><p class="marker"></p>商品的专柜价、吊牌价、正品零售价、厂商指导价或该商品的曾经展示过的销售价等。<span>并非原价</span>,仅供参考</div>
            </div>
            <div>
                <div class="words_explain_first"><i>&nbsp;</i> 未划线说明</div>
                <div class="words_explain_second"><p class="marker"></p> 商品的<span>实时标价</span>,不因表述的差异改变性质。具体成交价格根据商品参加活动,或会员使用优惠券、积分等发生変化,最终以订单结算页价格为准</div>
                <div class="words_explain_second"><p class="marker"></p> 商家详情页(含主图)以图片或文字形式标注的一口价、促销价、优惠价等价格可能是在使用优惠券、满减或特定优惠活动和时段等情形下的价格,具体请以结算页面的标价、优惠条件或活动规则为准。</div>
                <div class="words_explain_second"><p class="marker"></p> 此说明仅当出现价格比较时有效,具体请参见《价格发布规范》。若商家单独对划线价格进行说明的,以商家的表述为准。</div>
            </div>
        ...
        <yd-popup v-model="show2" position="bottom" height="60%" style="overflow-Y: auto;">
            ...
            <div class="pop-center">
                <div class="pop-header">
                    <img v-lazy="goodsFormsImg" alt="11" width="90" height="90" style="border-radius: 6px">
                    <div class="pop-r" style="width:100%;padding-left:14px">
                        <span style="color: red;">库存: {{goodsFormsStorageGoods}}</span>
                        <h1 style="font-size: 14px;color:#F5A623; font-weight:400">价格: {{countMoney}}</h1>
                        <div class="r">
                            <yd-spinner :max="goodsFormsStorageGoods" unit="1" v-model="countNumber"></yd-spinner>
                        </div>
                        <div>
                        </div>
                    </div>
                ...
                <div v-if="goodsForms.length!=0">
                    <p style="color: #333333;font-size:13px;">{{goodsDetail.formname}}</p>
                    <ul>
                        <li :class="{cricle:true,a:goodsFormsId==item.id}" style="float: left;width: auto;margin: 2px;border: 1px solid;padding: 2px 12px;" v-for="item in goodsForms" :key="item.id"
                        @click="goodsFormsClick(item)">{{item.name}}</li>
                    </ul>
                ...
                <div v-if="secondClass.length!=0">
                    ...
                    <ul>
                        <li :class="{cricle:true,a:secondClassId== key}" style="float: left;width: auto;margin: 2px;border: 1px solid;padding: 2px 12px;" v-for="(item,key) in secondClass"
                            @click="optionClick(key)">{{item}}</li>
                    </ul>
                    <div style="clear: left"></div>
                </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            try {
                sessionStorage.setItem("pMerchantno", this.$route.params.pMerchantno);
            } catch (e) {
            }
            //加载商品详情
            this.loadShop();
        ...
        methods: {
            appShare() {
                try {
                    ...
                    if (ua.match(/MicroMessenger/i) == 'micromessenger') {
                        ...
                        try {
                            JSHook.showMessage("http://mall.zybkeji.com/#/home/reservepage/"+this.$route.params.id+"/"+localStorage.getItem("oMerchantno"));
                        } catch (e) {
                        }
                        try {
                            window.webkit.messageHandlers.showMessage.postMessage(["http://mall.zybkeji.com/#/home/reservepage/"+this.$route.params.id+"/"+localStorage.getItem("oMerchantno")])
                        } catch (e) {
                        }
                    }
                } catch (e) {
            ...
            addCar() {
                ...
                if (this.isCar == 0) {
                    ...
                    sessionStorage.setItem("xdType", "zjxd");
